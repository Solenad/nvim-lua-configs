...
--     "folke/snacks.nvim",
--   },
--   config = function()                   -- Default configuration with all available options
...
return {
  ...
  config = function()
    vim.g.opencode_opts = {
      server = {
        -- Replace "4444" with your preferred port if needed
        args = { "--port", "4444" },
      },
      -- This ensures it actually tries to open the terminal buffer
      terminal = {
        extmark = { enabled = true },
      }
    }
    vim.o.autoread = true
    local opencode = require("opencode")

    -- Define your own custom Toggle function
    local function manual_opencode_toggle()
      -- Check if an opencode terminal is already open
      ...
      if terminal_buf then
        -- If it exists, toggle it: if it's visible, hide it; if not, show it
        ...
        -- We use "opencode --port 4444" (Add .exe if Windows complains)
        vim.cmd("vsplit | term opencode --port 4444")
        vim.cmd("setlocal nonumber norelativenumber")
        vim.cmd("startinsert") -- Drop straight into typing mode
      end
    end

    -- Bind your new manual toggle
    vim.keymap.set({ "n", "x" }, "<C-a>", function() require("opencode").ask("@this: ", { submit = true }) end,
      { desc = "Ask opencode…" })
    vim.keymap.set({ "n", "x" }, "<C-x>", function() require("opencode").select() end,
      { desc = "Execute opencode action…" })
    vim.keymap.set({ "n", "t" }, "<leader>O", manual_opencode_toggle, { desc = "Manual OpenCode Terminal" })
